...
public class PlayerService extends Service {
    ...
    private void setupMediaSession() {
        ...
        mediaSession.setCallback(new MediaSession.Callback() {
            ...
            @Override public void onSkipToNext() { requestSkipNext(); }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null && intent.getAction() != null) {
            switch (intent.getAction()) {
                ...
                case ACTION_NEXT: requestSkipNext(); break;
    ...
    /**
     * Мгновенно переключается на уже подготовленный (prepareNext) трек, если он есть.
     * Не зависит от того, работает ли сейчас JS в WebView — поэтому безопасно вызывать
     * из фона (по кнопке "следующий" в уведомлении/наушниках) и при естественном окончании трека.
     * Возвращает true, если переключение получилось.
     */
    private boolean advanceToPreparedNext() {
        if (!nextPrepared || nextPlayer == null) return false;
    ...
    /** Явное нажатие "следующий трек" (уведомление, наушники, шторка). */
    public void requestSkipNext() {
        if (!advanceToPreparedNext() && callback != null) {
            // Подготовленного трека ещё нет (редкий момент сразу после старта) — просим JS решить,
            // какой трек следующий, и загрузить его как обычно.
            callback.onRemoteNext();
        }
    }
    ...
    public void onTaskRemoved(Intent rootIntent) {
        // По умолчанию Android и так не останавливает сервис при закрытии задачи (свайп из
        // списка недавних приложений), но переопределяем явно и НЕ вызываем stopSelf(),
        // чтобы музыка гарантированно продолжала играть после закрытия приложения.
